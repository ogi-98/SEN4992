...
public struct Line: View {
    ...
    @State var gradientBottomColor: Color = Color.white
    var index: Int = 0
    ...
    public var body: some View {
        ZStack {
            if self.showFull && self.showBackground {
                self.closedPath
                    .fill(LinearGradient(gradient: Gradient(colors: [Colors.GradientUpperBlue, gradientBottomColor]), startPoint: .bottom, endPoint: .top))
                    .rotationEffect(.degrees(180), anchor: .center)
                    .rotation3DEffect(.degrees(180), axis: (x: 0, y: 1, z: 0))
                    .transition(.opacity)
                    .animation(.easeIn(duration: 1.0), value: customAnimationDuration)
                    .onAppear {
                        customAnimationDuration.toggle()
                    }
                    .onDisappear {
                        customAnimationDuration.toggle()
                    }
            ...
            self.path
                ...
                .rotationEffect(.degrees(180), anchor: .center)
                .rotation3DEffect(.degrees(180), axis: (x: 0, y: 1, z: 0))
                .animation(.easeOut(duration: 1.2).delay(Double(self.index) * 0.4), value: customAnimationDuration)
                .onAppear {
                    self.showFull = true
                    customAnimationDuration.toggle()
            }
